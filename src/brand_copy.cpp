...
BrandDescriptorExtractor_copy::BrandDescriptorExtractor_copy( double degree_threshold, int descriptor_size )
{
...
int BrandDescriptorExtractor_copy::getDescriptorSize() const
{
	return m_descriptor_size;
}

void BrandDescriptorExtractor_copy::compute(const cv::Mat& image, const cv::Mat& color, const cv::Mat& depth, const cv::Mat& cloud,
...
void BrandDescriptorExtractor_copy::canonical_orientation(  const cv::Mat& img, const cv::Mat& mask,
...
void BrandDescriptorExtractor_copy::compute_intensity_and_shape_descriptors( const cv::Mat& image,
																		const cv::Mat& color,
																		const cv::Mat& depth,
                                                                        const cv::Mat& cloud,
...
inline int BrandDescriptorExtractor_copy::smoothedSum(const cv::Mat& sum, const cv::Point2f& pt) const
{
...
inline float BrandDescriptorExtractor_copy::smoothedSumAngle(const cv::Mat& sum, const cv::Point2f& pt) const
{
...
void BrandDescriptorExtractor_copy::computeAngle(const cv::Mat& image, const cv::Mat& depth_img, std::vector<cv::KeyPoint>& kpts) const
{
	cv::Mat sum_depth;
...
void BrandDescriptorExtractor_copy::pixelTests(  const cv::Mat& sum,
