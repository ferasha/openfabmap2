...
int CDORB::descriptorType() const
{
    return CV_8U;
}

int CDORB::descriptorSize() const {
	return 32;
}
...
void CDORB::computeImpl( const cv::Mat& gray, std::vector<cv::KeyPoint>& keypoints,
...
void CDORB::computeAngle(const cv::Mat& gray, cv::KeyPoint& kpt) const
{
		int half_k = kpt.size / 2;
		...
		const uchar* center = &gray.at<uchar> (cvRound(kpt.pt.y), cvRound(kpt.pt.x));
		...
		kpt.angle = cv::fastAtan2((float)m_01, (float)m_10);

}

void CDORB::getPixelPairs(int index, cv::Mat& R, const cv::KeyPoint& kpt, float scale,
		cv::Point2f& p1, cv::Point2f& p2) const {

    p1.x = bit_pattern_31_[index * 4]     * scale;
    p1.y = bit_pattern_31_[index * 4 + 1] * scale;

    p2.x = bit_pattern_31_[index * 4 + 2] * scale;
    p2.y = bit_pattern_31_[index * 4 + 3] * scale;
...
inline int CDORB::smoothedSum(const cv::Mat& sum, const cv::Point2f& pt) const
{
...
void CDORB::computeDescriptors(const cv::Mat& gray, const cv::Mat& sum_depth, const std::vector<cv::Mat>& sum_color,
